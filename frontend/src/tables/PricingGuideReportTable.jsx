...
import {
  downloadPricingGuidePDF,
  downloadPricingGuideExcel,
...
const PricingGuideTable = ({ computedProducts }) => {
  ...
  const reportColumns = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <span className={`font-medium ${color}`}>₱{profit.toFixed(2)}</span>
        );
      },
    },
    {
  ...
  const toolbar = (
    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-2.5 mt-4">
      <SelectBox
