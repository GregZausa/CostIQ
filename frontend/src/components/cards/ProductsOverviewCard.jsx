...
const ProductsOverviewCard = ({ title, value, description, icon }) => {
  const { isDark } = useTheme();
  return (
    <div
      className={`group relative border ${isDark ? "bg-slate-800 border-slate-700 hover:border-slate-600" : "bg-slate-50 border-slate-200 hover:border-slate-200"} rounded-2xl p-2 shadow-xs hover:shadow-md transition-all duration-300 overflow-hidden`}
    >
